...
# We have an irritating number of site and configuration dependencies which are
# awkward to push down into the EPICS makefile because of how EPICS build works.
EPICS_DEPENDS =
EPICS_DEPENDS += epics/configure/RELEASE
EPICS_DEPENDS += $(DEFS_PATH)
EPICS_DEPENDS += epics/opi/mbf/overview.edl
EPICS_DEPENDS += epics/opi/scripts/site
...
clean-epics:
	...
	rm -f epics/opi/scripts/site
.PHONY: clean-epics


# This is a bit excessive, as configure/RELEASE isn't a real dependency, but
